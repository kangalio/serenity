...
async fn handle_event(
    ...
    match model_event {
        ...
        Event::ChannelDelete(mut event) => {
            let cached_messages = if_cache!(update(&context, &mut event));
        ...
        Event::GuildDelete(mut event) => {
            let full = if_cache!(update(&context, &mut event));
        ...
        Event::GuildMemberRemove(mut event) => {
            let member = if_cache!(update(&context, &mut event));
        ...
        Event::GuildMemberUpdate(mut event) => {
            let before = if_cache!(update(&context, &mut event));
        ...
        Event::GuildRoleDelete(mut event) => {
            let role = if_cache!(update(&context, &mut event));
